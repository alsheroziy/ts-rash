...
export class PDFService {
  ...
  static async generateRashModelPDF_HTML(testId?: string): Promise<Buffer> {
    if (testId) {
      ...
      const rows = [] as Array<{ no: number; name: string; correct: number; ball: string; grade: string }>;
      for (let i = 0; i < results.length; i++) {
        const r = results[i];
        let user: any = null;
        const userIdNum = Number(r.userId);
        if (!isNaN(userIdNum)) user = await User.findOne({ telegramId: userIdNum });
        else user = await User.findById(r.userId);
    ...
    // All results (compact)
    const results = await TestResult.find({}).populate('testId', 'title').sort({ rashScore: -1 }).limit(100);
    const rows = [] as Array<{ no: number; name: string; correct: number; ball: string; grade: string }>;
    for (let i = 0; i < results.length; i++) {
  ...
  static async generateRashModelPDF(testId?: string): Promise<Buffer> {
    ...
    if (testId) {
      ...
      doc.setFontSize(11);
      doc.text(
        "DAVRON AKBAR TOMONIDAN O'TKAZILGAN MILLIY SERTIFIKAT IMTIHONIDA TALABGORLARNING RASH MODELI BO'YICHA TEKSHIRILGAN TEST NATIJALARI",
        105,
        yPosition + 8,
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (!isNaN(userIdNum)) {
          user = await User.findOne({ telegramId: userIdNum });
        } else {
          // Fallback: try to find by _id if userId is an ObjectId
          user = await User.findById(result.userId);
        }
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        const testTitle = (result.testId as any)?.title || 'Noma\'lum test';
      ...
      const avgRashScore = await TestResult.aggregate([
  ...
  static async generateResultsPDFWithNames(testId?: string): Promise<Buffer> {
    ...
    if (testId) {
      ...
      const totalResults = await TestResult.countDocuments();
      const avgPercentage = await TestResult.aggregate([
